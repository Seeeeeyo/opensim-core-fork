...
- The default value `OPENSIM_DISABLE_LOG_FILE` has been changed to `ON`, meaning that `opensim.log` will no longer be generated by default as part of the logging system. File logging ca be manually be enabled by calling `Logger::addFileSink()`. (#4380)
